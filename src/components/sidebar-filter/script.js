...
window.addEventListener( 'load', function( event ) {
  ...
  $('.btn-filter-submit').on('click touch',function () {
    $('body').removeClass('filter-active');
  });
  $(document).on('click touch',function (e) {
    let _target = $(e.target);
    if(
        _target.hasClass('.sidebar-filter-wrap')||
        _target.closest('.sidebar-filter-wrap').length
    ) {
      return
    }
    $('body').removeClass('filter-active');
    $('.sidebar-filter-wrap').removeClass('active');
    $('.js-open-filters').removeClass('active');
  });
